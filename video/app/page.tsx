...
export default function HeartProject() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      if (r < 0.25) {
        // Photo — remplace le src par tes vraies images
      const photos = [
         '/image/photo.png',
         '/image/photo2.png',
         '/image/photo3.png',
     ...
     const img = document.createElement('img');
        ...
        el.style.webkitTextStroke = '2px #ff69b4';
    ...
    function animate(ts: number) {
      ...
      for (const it of items) {
        ...
        it.el.style.transform = `translate3d(${it.x}px, ${it.y}px, ${it.z}px) rotateZ(${it.rotZ}deg)`;
        const progress = (it.z + 900) / 1500;
        it.el.style.opacity = String(Math.min(1, Math.max(0, progress * 1.5)));
      }
  ...
  return (
    <main className="w-full h-screen bg-black overflow-hidden" style={{ perspective: '400px' }}>
      <div
        ref={worldRef}
